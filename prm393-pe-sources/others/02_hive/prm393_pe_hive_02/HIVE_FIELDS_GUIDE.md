...
### 7.1 Search Products (Tìm kiếm sản phẩm)

**DatabaseService** - Thêm method:
...
### 7.2 Filter by Category (Lọc theo danh mục)
...
### 7.3 Filter by Price Range (Lọc theo khoảng giá)
...
### 7.4 Update Product (Cập nhật sản phẩm)

**DatabaseService** - Method đã có:
...
### 7.5 Get Product by ID (Lấy sản phẩm theo ID)
...
// Tìm kiếm
await productProvider.searchProducts('smartphone');

// Lọc theo danh mục
await productProvider.filterByCategory('electronics');
...
// Xóa bộ lọc
await productProvider.clearFilters();
